...
const ShowOurServices = () => {
  ...
  return (
    // Stack vertically on mobile (flex-col), side-by-side on large screens (lg:flex-row)
    <div className="flex flex-col lg:flex-row items-start justify-between mt-8 lg:mt-16 gap-6 lg:gap-10 px-4 lg:px-0">
        ...
        <section className="w-full lg:w-[90%] mx-auto mt-6">
          {activeCategory && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="relative w-full aspect-video lg:h-[500px] rounded-2xl overflow-hidden shadow-lg">
                <img
              ...
              {/* Service Cards Grid - 1 col on mobile, 2 on tablet/desktop */}
              <section className="mt-8 lg:mt-10 grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-10">
                {activeCategory?.ServicesName?.map((service, id) => (
                  <div
                    key={id}
                    className="p-6 border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all bg-white flex flex-col gap-3 group"
                  >
                    <div className="text-orange-500 animate-pulse  bg-orange-50 w-fit p-3 rounded-lg group-hover:bg-orange-500 group-hover:text-white transition-colors">
                      {service.icon}
                    </div>
                    ...
                    <Link
                      href={service.link}
                      className="text-orange-600 text-sm font-semibold hover:underline mt-2 inline-flex items-center"
                    >
                      Learn More →
                    </Link>
                  </div>
      ...
      {/* Right Column (Sidebar/Accordion) */}
      <div className="w-full lg:w-1/3 lg:sticky lg:top-24 order-1 lg:order-2">
        <div className="bg-white p-4 flex flex-col gap-y-3 lg:gap-y-4 shadow-xl lg:shadow-lg rounded-2xl border border-gray-50">
              ...
              <div key={idx} className="w-full">
                ...
                >
                  <div className="overflow-hidden">
                    <div className="p-4 flex flex-col gap-3 bg-white rounded-xl border border-gray-100 mx-1">
          ...
          <button className="mt-2 text-lg justify-center flex items-center gap-x-3 text-white bg-[#102a42] w-full p-4 rounded-xl transition-all hover:bg-orange-600 active:scale-95 shadow-md">
            <FaFilePdf />
            Company Brochure
          </button>
        </div>
